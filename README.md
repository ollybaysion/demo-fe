...
채팅 인터페이스 위에 데이터 요청 왕복(요청 카드 ↔ 붙여넣기), 설비 카드,
운영자 인계용 요약, 이미지 첨부, 5개 컬러 테마 등 도메인 워크플로우를 얹은
Next.js App Router 앱이다. 백엔드는 별도 — 본 저장소는 클라이언트 + mock
SSE 라우트만.
...
│   │   ├── context/             # 우측 설비 패널 (데이터에서 파생된 설비 카드)
│   │   ├── data/                # 좌측 데이터 패널 (요청 카드 · 스냅샷 그룹)
...
- **운영자 요약** — 우측 320px 패널, 컨텍스트 자동 동봉, 마크다운으로
  클립보드 복사.
...
- **각 `route.ts`** 는 `BACKEND_URL` 환경 변수 한 곳으로 동작 모드를 결정 — `src/lib/backend.ts` 의 `forwardOrMock` helper 가 일괄 처리:
  - `BACKEND_URL` **미설정** → route 안의 mock 응답 (`src/demo/` 에서 import).
