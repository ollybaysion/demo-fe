...
> 정형 조회 4종(`GET /equipment/:id` · `/peers` · `/setup-events` · `/compare`)은
> **2026-07-19 계약에서 제거** — 설비 상세·비교는 챗 에이전트로 일원화했다.
> 과거 스펙은 이 문서의 git 이력 참조 (§[변경 이력](#변경-이력-스펙)).
...
- **2026-07-19** — 정형 조회 4종(`GET /equipment/:id`·`/peers`·`/setup-events`·`/compare`) 계약 제거.
  설비 상세·비교는 챗 에이전트로 일원화. 과거 스펙 원문은 git 이력, 구현은 deprecated
  `fdc-agent-be`(Node 판) 이력에 보존.
