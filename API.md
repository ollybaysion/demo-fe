...
type ChatDataSnapshot = {
  ...
  rowCount: number;
  /**
   * 표 전문. 값은 전부 원문 문자열이고, NULL 은 빈 문자열과 구별해 `null` 이다.
   *
   * **빈 배열은 "내용 없음"이 아니라 "조회 결과가 0행"이다** — 아래 세 상태 표 참고.
   */
  rows?: (string | null)[][];
};
```
...
내용 전달은 **세 갈래**다. 이 구분이 이 계약의 요점이다.

| 상태 | `rows` | `rowCount` | 의미 |
| --- | --- | --- | --- |
| 카탈로그 | 없음 | > 0 | "이런 표가 있다"만 알린다. 내용은 아직 안 왔고, 다시 요청할 수 있다 |
| **조회 결과 0행** | `[]` | `0` | **조회했고 결과가 없었다.** 미첨부가 아니라 "없다"는 **사실**이다 |
| 내용 동봉 | 있음 | 행 수 | 사용자가 "이건 보고 답해라"로 체크한 것 |

**0행을 카탈로그와 뭉치지 말 것.** 뭉치면 조회해서 없음을 확인한 사용자에게 같은 요청이
다시 나가거나, 모델이 오지 않을 데이터를 기다린다. 0행이 도착하면 그 조회 절차는 거기서
정상 종료되고, 답은 "없다"가 된다. `rows` 를 아예 안 싣는 클라이언트를 위해
`rowCount === 0` 도 같은 뜻으로 받는다.

카탈로그가 따로 있는 이유는 크기다 — 표 하나가 수천 행일 수 있어 전부 실으면 요청이 감당이
...
type DonePayload = {
   ...
   * 조회할 수 없을 때, 지어내는 대신 사용자에게 조달을 요청하는 통로.
   */
...
type DataRequest = {
  /**
   * 이 요청을 충족하는 스냅샷이 가질 키. 사용자가 카드에서 등록하면 이 값이 그대로
   * `dataSnapshots[].queryKey` 가 되어, 다음 요청에서 충족 여부를 알아볼 수 있다.
   */
  queryKey: string;
  /** 사람이 읽는 설명 — 무슨 데이터가 왜 필요한지. */
  label: string;
  /** 사용자가 실행할 SQL. 있으면 카드가 복사 버튼을 준다. */
  sql?: string;
  /** 기대 컬럼. 맞는 걸 붙여넣었는지 사용자가 가늠하게 한다. */
  columns?: string[];
};
```
...
#### `dataRequests` 를 보내는 쪽에 대하여

이 필드는 **`dataSnapshots` 와 한 쌍**이다. 요청을 내면 사용자가 채우고, 채워진 것이 다음
요청의 `dataSnapshots` 로 돌아온다. 그 왕복이 이 기능의 전부다.

- **`queryKey` 는 안정적이어야 한다.** 사용자가 등록한 스냅샷이 그 키를 갖게 되므로, 같은
  데이터를 다시 요구할 때 키가 달라지면 이미 받은 것을 또 요구하게 된다.
- **없는 데이터를 추정해 답하지 말고 요청한다.** 요청 카드가 존재하는 이유가 그것이다.
- FE 는 요청 카드를 **마지막 어시스턴트 메시지에만** 그린다. 지난 턴의 요청은 이미
  채워졌거나 사용자가 지나간 것이라, 남겨 두면 대화가 낡은 요구로 뒤덮인다.
- 사용자가 카드로 채운 스냅샷은 **동봉 ON 으로 등록**된다. 이미 "필요하다"는 요구에 대한
  응답이라 꺼진 채로 시작할 이유가 없다.
